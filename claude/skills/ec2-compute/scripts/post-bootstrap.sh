...
#   AGENT_USER      — system user (ubuntu or ec2-user)
#   USER_HOME       — home directory (/home/ubuntu or /Users/ec2-user)
set -euo pipefail
...
# Install Tailscale only if not already present (stock AMI or macOS path)
...
sudo -u "$AGENT_USER" bash -c "git config --global url.\"https://x-access-token:${GITHUB_TOKEN}@github.com/\".insteadOf \"https://github.com/\""

# Authenticate gh CLI
echo "$GITHUB_TOKEN" | sudo -u "$AGENT_USER" bash -c 'gh auth login --with-token'

# Store OP token for agent user (so they can use op read)
echo "export OP_SERVICE_ACCOUNT_TOKEN=$OP_SERVICE_ACCOUNT_TOKEN" >> "$USER_HOME/.zshrc"
if [[ -f "$USER_HOME/.bashrc" ]]; then
  echo "export OP_SERVICE_ACCOUNT_TOKEN=$OP_SERVICE_ACCOUNT_TOKEN" >> "$USER_HOME/.bashrc"
fi
